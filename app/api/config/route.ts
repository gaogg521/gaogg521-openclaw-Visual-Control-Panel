...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: readErr?.message || "Failed to read config",
        errorCode: isNotFound ? "CONFIG_NOT_FOUND" : "CONFIG_READ_ERROR",
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    // 每个模型行：哪些 Agent 的「主模型」配置（解析后）等于 provider/modelId
    for (const target of providers) {
      const pid = target.id;
      if (!Array.isArray(target.models)) continue;
      for (const m of target.models) {
        const ref = `${pid}/${m.id}`;
        m.usedBy = agentsWithStatus
          .filter((a: any) => typeof a.model === "string" && a.model.trim() === ref)
          .map((a: any) => ({ id: a.id, emoji: a.emoji, name: a.name }));
      }
    }
    ...
    return NextResponse.json(
      { error: err?.message || "Config processing failed", errorCode: "CONFIG_ERROR" },
      { status: 500 }
    );
  }
}
